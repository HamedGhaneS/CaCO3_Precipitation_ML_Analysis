...
class YiPredictionAnalyzer:
    ...
    def create_3d_surface_plots(self, system_name):
        """Create clean 3D surface plots based on the two most dominant Xi descriptors"""
        ...
        # Create separate figure for each target (Yi)
        for target_idx, (target, model_info) in enumerate(system_models.items()):
            ...
            # Create separate figure for this target
            ...
            x1_extended_min = x1_min - 0.1 * x1_range
            x1_extended_max = x1_max + 0.1 * x1_range
            x2_extended_min = x2_min - 0.1 * x2_range
            x2_extended_max = x2_max + 0.1 * x2_range
            
            # Create mesh grid for surface (use reasonable resolution)
            x1_mesh = np.linspace(x1_extended_min, x1_extended_max, 25)
            x2_mesh = np.linspace(x2_extended_min, x2_extended_max, 25)
            ...
            # Plot surface with better styling
            surf = ax.plot_surface(X1_grid, X2_grid, Z_pred, 
                                 cmap='viridis', alpha=0.8, 
                                 linewidth=0.5, antialiased=True,
                                 edgecolors='white', linewidths=0.1)
            
            # Plot actual data points with better visibility
            scatter = ax.scatter(x1_values, x2_values, y_values, 
                               c='red', s=80, alpha=1.0, 
                               edgecolors='black', linewidth=1.0,
                               label='Measured Data', depthshade=True)
            
            # Clean axis labels
            param1_name = param_names.get(top_feature_1, top_feature_1)
            param2_name = param_names.get(top_feature_2, top_feature_2)
            
            ax.set_xlabel(f'\n{param1_name}', fontsize=12, labelpad=10)
            ax.set_ylabel(f'\n{param2_name}', fontsize=12, labelpad=10)
            ax.set_zlabel(f'\n{model_info["target_name"]}', fontsize=12, labelpad=10)
            
            # Clean title
            importance_1 = importance_pairs[0][1]
            importance_2 = importance_pairs[1][1]
            
            ax.set_title(f'{system_name} System - {model_info["target_name"]}\n'
                        f'Most Dominant Parameters: {param1_name} ({importance_1:.3f}) & {param2_name} ({importance_2:.3f})\n'
                        f'Model: {model_info["best_name"]}',
                        fontsize=14, pad=20)
            
            # Add colorbar with proper positioning
            cbar = fig.colorbar(surf, ax=ax, shrink=0.6, aspect=30, pad=0.1)
            cbar.set_label(model_info["target_name"], rotation=270, labelpad=20, fontsize=11)
            ...
            target_clean = target.replace('_', '').replace('%', 'Percent').replace('(', '').replace(')', '').replace('°', 'deg')
            filename = f'H:/FreeLancer/Elmira/{system_name}_{target_clean}_3D_surface.png'
            plt.savefig(filename, dpi=300, bbox_inches='tight')
            plt.show()
            
            print(f"    Saved: {filename}")
        
        # Create a summary plot with all 3 targets in one figure
        self.create_3d_summary_plot(system_name)
    ...
    def run_complete_analysis(self):
        ...
        print("- Calcium_3D_surfaces.png (Ca 3D surface plots)")
        print("- Sodium_3D_surfaces.png (Na 3D surface plots)")
        print("- Feature_Importance_Analysis.png (Xi dominance analysis)")
        print("\nYou can now predict Y1, Y2, Y3 for any Xi combination within the bounds!")
        print("3D plots show Yi response based on the TWO MOST DOMINANT Xi descriptors!")
